...
class ProductosDesdeExcelSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (!is_array($rows)) {
            $this->command?->error('El JSON de entrada es inválido.');
            return;
        }
        ...
        foreach ($rows as $row) {
            ...
            $categoriaNombre = $this->cleanText($row['Categoría'] ?? null) ?? 'Sin categoria';
            $tipo = $this->cleanText($row['Tipo'] ?? null);
            $presentacionRaw = $this->cleanText($row['Presentación'] ?? null);
